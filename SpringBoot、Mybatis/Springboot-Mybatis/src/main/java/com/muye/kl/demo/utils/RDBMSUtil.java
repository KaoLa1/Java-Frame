...
public class RDBMSUtil {
     ...
     * @return
     */
    public static Connection getConn(DataSourceInfo dataSourceInfo) {
     ...
     * @param dataSourceInfo
     */
    public static void closeConn(DataSourceInfo dataSourceInfo){
        Connection conn = getConn(dataSourceInfo);
        if(conn!=null){
            try {
                conn.close();
                log.info("关闭数据库连接");
     ...
     * 获取数据库下的所有表名
     *
     * @param dataSourceInfo   资源实体
     * @return tableList
     */
    public static List<String> buildDatabaseMetaDataTables(DataSourceInfo dataSourceInfo) throws SQLException {
//        DataSource dataSource = getDataSource(driver, url, username, password);
        ...
        try {
            ...
            tableList = new ArrayList<>(resultSet.getRow());
     ...
     * @return
     */
    public static List<Metadata> buildTableMetadata(DataSourceInfo dataSourceInfo, String tableName) throws SQLException {
//        DataSource dataSource = getDataSource(driver, url, username, password);
        Connection conn = getConn(dataSourceInfo);
    ...
    public static Information getInformationFromUrl(String url) {
        if (org.apache.commons.lang3.StringUtils.isEmpty(url)) return null;
